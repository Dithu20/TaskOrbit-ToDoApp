...
export default function AddTaskScreen({ navigation }: any) {
  ...
  const validate = () => {
    ...
    if (title.trim().length < 3) {
      Alert.alert("Serious?", "Minimum 3 letters");
      return false;
    }
    return true;
  };
  ...
  const saveTask = useCallback(async () => {
    ...
    if (!user) {
      Alert.alert("Login First", "system");
      return;
    }
    ...
    try {
      ...
      Alert.alert("this a Reason!", `"${title}" saved successfully!`, [
        {
          text: "text",
          onPress: () => {
            resetForm();
      ...
      Alert.alert("Something went wrong", error?.message || "Something went wrong!");
    } finally {
      setLoading(false);
    }
          ...
          >
            {loading ? (
              ...
              <>
                ...
                <Text style={styles.saveBtnText}>Save Task - Style</Text>
              </>
            )}
          </TouchableOpacity>
        </ScrollView>
